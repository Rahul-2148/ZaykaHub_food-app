...
export const createRestaurant = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Handle cuisines coming either as a JSON string or an array
    ...
    console.log(err);
    if (uploadedImageUrl) {
      const publicIdMatch = uploadedImageUrl.match(/\/v\d+\/(.+)\./);
    ...
    if (file) {
      try {
        ...
        if (restaurant.imageUrl) {
          const prevId = restaurant.imageUrl.split("/").pop()?.split(".")[0];
          if (prevId) {
            await cloudinary.uploader.destroy(`ZaykaHub/restaurants/${prevId}`);
          }
        }
...
export const searchRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: "Failed to search restaurants" });
  }
};
...
export const getSingleRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    const restaurant = await Restaurant.findById(restaurantId)
      ...
      .populate("user", "fullname email") // Fetch restaurant owner details
      .populate({
        path: "reviews.user",
        select: "fullname email",
      });
...
export const getAllRestaurants = async (req: Request, res: Response) => {
  try {
    ...
    const restaurants = await Restaurant.find({})
      .populate("user", "fullname email") // Fetch owner details
...
export const deleteRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    const menus = await Menu.find({ _id: { $in: restaurant.menus } }).select(
      "image",
    );

    // Delete related data concurrently using Promise.all()
    await Promise.all([
      // Menus are stored as IDs inside restaurant.menus (menu documents don't have restaurant ref)
      Menu.deleteMany({ _id: { $in: restaurant.menus } }),
      Order.deleteMany({ restaurant: restaurant._id }),
      Review.deleteMany({ restaurant: restaurant._id }),
      restaurant.imageUrl
        ? cloudinary.uploader.destroy(
            `ZaykaHub/restaurants/${restaurant.imageUrl.split("/").pop()?.split(".")[0]}`,
          )
        : Promise.resolve(),
      Promise.all(
        menus.map((menu) => {
          if (!menu.image) return Promise.resolve();
          const match = menu.image.match(/\/v\d+\/(.+)\./);
