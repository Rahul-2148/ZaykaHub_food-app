...
import uploadImageOnCloudinary, { uploadUserProfileImage } from "../utils/imageUpload";
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    await sendPasswordResetEmail(
      user.email,
      user.fullname,
      `${process.env.FRONTEND_ORIGIN || process.env.FRONTEND_URL || "http://localhost:5173"}/reset-password/${resetPasswordToken}`
    );
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    console.log("Sending reset success email to:", user.email);
...
export const updateProfile = async (req: Request, res: Response) => {
  try {
      ...
      try {
        ...
        if (existingUser?.profilePicture) {
          const publicId = existingUser.profilePicture
            ...
            ?.split(".")[0];
          if (publicId) {
            await cloudinary.uploader.destroy(`ZaykaHub/users/${publicId}`);
          }
        }

        // Upload new profile picture with user's name
        const userName = fullname || existingUser?.fullname || "user";
        profilePictureUrl = await uploadUserProfileImage(req.file as Express.Multer.File, userName);
      } catch (error) {
    ...
    // old profile picture already deleted before upload above
...
export const EmailUnSubscribe = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: "Failed to unsubscribe email" });
  }
};
...
export const EmailReSubscribe = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: "Failed to resubscribe email" });
  }
};
